...
13. **Localized screenshots** — "Do you want screenshots in multiple languages? (default: always use locale folders even for single-language apps — just one `en/` folder is fine). If adding more languages, which ones? (e.g. zh, de, es, ja, ar)"
...
- **Locale folder structure**: always use `screenshots/{locale}/` even for single-language apps (default locale is `en`)
...
│   └── screenshots/
│       ├── en/                 # Always use locale subfolders, even for single-language apps
│       │   ├── home.png
│       │   ├── feature-1.png
│       │   └── ...
│       ├── zh/                 # Add more locales here as needed
│       └── {locale}/
├── src/app/
...
### Multi-language: Locale Select + COPY Object

Add a `LOCALES` array, a `COPY` object with all slide text keyed by locale, and a `<select>` locale picker in the toolbar. Every slide receives `locale` as a prop and pulls text from `COPY[locale]`. Image paths also include the locale subdirectory — no hardcoded locale anywhere in slide components.

**Even for single-language apps, always use the locale-based folder structure** (`screenshots/en/home.png`) — it costs nothing and makes adding a second language trivial later.

#### LOCALES + Locale type

```tsx
const LOCALES = ["en", "zh"] as const; // extend with more locales as needed
type Locale = (typeof LOCALES)[number];
```

#### COPY object — all slide text keyed by locale

Define a `COPY` record typed against the full slide set. Each locale entry must have identical keys — TypeScript enforces this.

```tsx
const COPY: Record<
  Locale,
  {
    s1: { label: string; headline: React.ReactNode; sub: string };
    s2: { label: string; headline: React.ReactNode; sub: string };
    // ... one entry per slide, with any slide-specific extra fields
    // e.g. s3: { ...; badges: { icon: string; label: string }[] }
    //      s6: { ...; installing: string }
    //      s9: { ...; stats: { value: string; label: string }[] }
    sLast: { subLabel: string; headline: React.ReactNode; features: string[]; comingSoon: string[]; cta: string };
  }
> = {
  en: {
    s1: {
      label: "Turn your iPhone into an ADB controller",
      headline: <>ADB.<br />From your<br />pocket.</>,
      sub: "Connect to any Android device over Wi-Fi. No computer. No cable.",
    },
    // ... remaining slides
    sLast: {
      subLabel: "FOR DEVELOPERS",
      headline: <>The ADB tool<br />devs deserve.</>,
      features: ["ADB over Wi-Fi", "Shell commands", "APK sideload", "Logcat viewer", "Quick connect"],
      comingSoon: ["Screen mirror", "Battery monitor"],
      cta: "Available on the App Store",
    },
  },
  zh: {
    s1: {
      label: "用 iPhone 掌控 Android 设备",
      headline: <>ADB。<br />装进<br />口袋。</>,
      sub: "Wi-Fi 直连 Android 设备，无需电脑，无需数据线。",
    },
    // ... remaining slides in Chinese
    sLast: {
      subLabel: "专为开发者",
      headline: <>开发者值得<br />拥有的 ADB 工具。</>,
      features: ["Wi-Fi ADB", "Shell 命令", "APK 安装", "Logcat 查看", "快速连接"],
      comingSoon: ["屏幕镜像", "电量监控"],
      cta: "在 App Store 下载",
    },
  },
};
...
**Rules for COPY:**
- `headline` is always `React.ReactNode` so JSX `<br />` line breaks can be used
- Re-write headlines for each language — do not literally translate (German/Chinese often need shorter claims)
- For RTL locales (`ar`, `he`), set `dir="rtl"` on the canvas div and mirror any asymmetric layouts
- Slide-specific extra fields (badge labels, stat labels, progress text, etc.) must also be in COPY

#### SlideProps — always include locale
...
#### In each slide component — destructure locale, derive COPY and path

```tsx
const slide1: SlideDef = {
  id: "01-hero",
  component: ({ cW, cH, locale }) => {
    const fw = phoneW(cW, cH) * 100;
    const c = COPY[locale].s1;         // ← pull copy for this slide
    return (
      <SlideBase ...>
        <Caption cW={cW} label={c.label} headline={c.headline} sub={c.sub} />
        <Phone
          src={img(`/screenshots/${locale}/home.png`)}  // ← locale in path
          alt="Home screen"
          placeholderLabel={`public/screenshots/${locale}/home.png`}
          style={{ ... }}
        />
      </SlideBase>
    );
...
#### IMAGE_PATHS — preload all locales at once
...
const IMAGE_PATHS = [
  ...
  "/app-icon.png",
  ...(LOCALES).flatMap((loc) => [
    `/screenshots/${loc}/home.png`,
    `/screenshots/${loc}/remote.png`,
    `/screenshots/${loc}/commands.png`,
    // ... all screenshot filenames used across slides
  ]),
];
```
...
>
  ...
  {/* Add other locales with flag emoji for clarity */}
</select>
```

#### Pass locale to preview and offscreen renders

Both the preview grid cards and the offscreen export divs must receive `locale`:

```tsx
// Preview grid
<ScreenshotPreview ... locale={locale} />

// ScreenshotPreview signature
function ScreenshotPreview({ slide, cW, cH, locale, exportRefCallback, onExport }: {
  slide: SlideDef;
  cW: number;
  cH: number;
  locale: Locale;
  exportRefCallback: (el: HTMLDivElement | null) => void;
  onExport: () => void;
}) {
  // ...
  <slide.component cW={cW} cH={cH} locale={locale} />
}

// Offscreen export renders
{SLIDES.map((slide, i) => (
  <div key={slide.id} ref={(el) => { exportRefs.current[i] = el; }}
    style={{ width: W, height: H, position: "absolute", left: "-9999px" }}>
    <slide.component cW={W} cH={H} locale={locale} />
  </div>
))}
```

#### Export filenames — include locale

```tsx
a.download = `${String(i + 1).padStart(2, "0")}-${slides[i].id}-${locale}-${size.w}x${size.h}.png`;
```

#### Setup banner — locale-aware folder instructions

```tsx
<div style={{ background: "#fffbeb", borderBottom: "1px solid #fde68a", padding: "10px 20px", fontSize: 12, color: "#92400e", fontFamily: "monospace" }}>
  <strong>Setup:</strong> Drop screenshots into{" "}
  {LOCALES.map(l => <><code>public/screenshots/{l}/</code>{" "}</>)}
  — filenames: <code>home.png</code>, <code>feature.png</code>, etc.
  Place app icon at <code>public/app-icon.png</code>.
  Refresh after adding files. Use the locale selector to switch languages.
</div>
```
...
App Store and Google Play both **reject screenshots with alpha channels** (transparency). `html-to-image`'s `toPng()` preserves transparency by default — any slide with a non-100%-opaque background will produce a PNG with alpha. Worse, browser `canvas.toDataURL("image/png")` still emits an RGBA PNG even if every pixel is opaque.

**Fix:** Use `toCanvas` to render, composite onto a fully opaque canvas, then re-encode the pixels with `fast-png` using `channels: 3`. That produces a true RGB PNG with no alpha channel in the file.

```typescript
import { encode } from "fast-png";
import { toCanvas } from "html-to-image"; // ← use toCanvas, not toPng

async function pngBytesToDataUrl(bytes: Uint8Array): Promise<string> {
  return await new Promise((resolve, reject) => {
    const reader = new FileReader();
    const buffer = new ArrayBuffer(bytes.byteLength);
    new Uint8Array(buffer).set(bytes);
    reader.onload = () => resolve(String(reader.result));
    reader.onerror = () => reject(reader.error ?? new Error("Failed to encode PNG data URL"));
...
async function captureSlide(el: HTMLElement, w: number, h: number): Promise<string> {
  ...
  try {
    ...
    const srcCanvas = await toCanvas(el, opts);

    // Flatten alpha first so stray transparent pixels resolve against black.
    const flat = document.createElement("canvas");
    flat.width = w;
    flat.height = h;
    const flatCtx = flat.getContext("2d");
    if (!flatCtx) throw new Error("Failed to create export canvas context");
    flatCtx.fillStyle = "#000000";
    flatCtx.fillRect(0, 0, w, h);
    flatCtx.drawImage(srcCanvas, 0, 0);

    // IMPORTANT: canvas.toDataURL("image/png") still writes RGBA PNGs.
    // Re-encode to a real RGB PNG by dropping alpha and using fast-png.
    const rgba = flatCtx.getImageData(0, 0, w, h).data;
    ...
    const pngBytes = encode({ width: w, height: h, data: rgb, channels: 3 });
    return await pngBytesToDataUrl(pngBytes);
  } finally {
    el.style.left = prevLeft || "-9999px";
...
**Why black for the fill?** Slide backgrounds are always opaque (dark or light gradient). In practice no transparent pixels should appear in the final slide — but if they do (from a rendering glitch), black is the safest fallback and avoids white halos around soft shadows.
...
- **No alpha channel**: Use `toCanvas` (not `toPng`), flatten onto black, then re-encode with `fast-png` using `channels: 3`. `canvas.toDataURL("image/png")` alone is not enough.
- **Double-call trick**: First `toCanvas()` loads fonts/images lazily. Second produces clean output. Without this, exports are blank.
- **On-screen for capture**: Temporarily move to `left: 0` before `toCanvas` — offscreen elements don't render.
...
### Export Quality
- **No alpha channel**: inspect the exported PNG metadata or reopen it in a PNG parser and confirm it is RGB / color type 2, not RGBA / color type 6. Visual opacity alone is not enough.
...
| Screenshots rejected by App Store / Google Play | Exported PNGs still contain an alpha channel — use `toCanvas` + black flatten + `fast-png` with `channels: 3`; `canvas.toDataURL("image/png")` still writes RGBA |
| Android tablet orientation ignored | Derive `cW/cH/slides` from `device + orientation` combo, not just `device` |
| Locale selector not working | `SlideProps` must include `locale: Locale`; pass it in both preview and offscreen export renders |
| Screenshots show wrong language | Image paths must use `${locale}` variable, not hardcoded locale string |
| Adding a language breaks layout | Test every locale at arm's length — re-write copy if translation is too long, don't just resize font |
| Single-language app, no locale folder | Always use `screenshots/en/` even for English-only — makes adding languages trivial later |
